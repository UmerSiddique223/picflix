...
function CreateStory() {
  ...
  return (
    <Dialog>
      <DialogTrigger className="absolute bottom-[77%] lg:fixed lg:bottom-4 right-4 bg-primary hover:brightness-90 p-3 rounded-lg">
        <Image
      ...
      <DialogContent>
        ...
        <DialogDescription>
          ...
          {error && (
            <p
              className={`font-bold ${
                error.status === "error" ? "text-destructive" : "text-green-400"
              }`}
            >
              {error.message}
            </p>
